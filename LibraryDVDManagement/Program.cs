...
class Program
{
            ...
            while (true)
            {
                ...
                switch (choice)
                {
                    case "1":
                        ...
                        break;
                    case "2":
                        RemoveMovie(movieCollection, memberCollection);
                        break;
                    case "3":
    ...
    static void AddMovie(MovieCollection movieCollection)
    {
        try
        {
            ...
            if (movieCollection.GetMovieByTitle(title) != null)
            {
                Console.WriteLine("Movie with this title already exists. Adding copies instead.");
                Console.Write("Enter number of copies to add: ");
                if (!int.TryParse(Console.ReadLine()?.Trim(), out int additionalCopies) || additionalCopies < 0)
                {
                    Console.WriteLine("Invalid number of copies. Please enter a positive number.");
                    return;
                }
                movieCollection.GetMovieByTitle(title).AddCopies(additionalCopies);
            ...
            if (!int.TryParse(Console.ReadLine()?.Trim(), out int duration) || duration <= 0 || duration > MaxDuration)
            {
            ...
            if (!int.TryParse(Console.ReadLine()?.Trim(), out int copies) || copies < 0)
            {
                Console.WriteLine("Invalid number of copies. Please enter a positive number.");
                return;
            }
    ...
    static void RemoveMovie(MovieCollection movieCollection, MemberCollection memberCollection)
    {
        try
        {
            Console.Write("Enter movie title: ");
            ...
            if (memberCollection.IsMovieBorrowed(title))
            {
                Console.WriteLine("Movie cannot be removed as it is currently borrowed by members.");
                return;
            }

            Console.Write("Enter number of copies to remove: ");
            if (!int.TryParse(Console.ReadLine()?.Trim(), out int copies))
            {
    ...
    static void RegisterMember(MemberCollection memberCollection)
    {
        try
        {
            ...
            {
                Console.WriteLine("First name cannot contain numbers.");
                return;
            }
            ...
            {
                Console.WriteLine("Last name cannot contain numbers.");
                return;
            }

            if (memberCollection.FindMemberByName(firstName, lastName) != null)
            {
                Console.WriteLine("Member with this name already exists. Please use a different name.");
                return;
            }

            Console.Write("Enter contact number (minimum 10 digits): ");
            string contactNumber = Console.ReadLine()?.Trim();
            if (contactNumber.Length < 10 || !contactNumber.All(char.IsDigit) || memberCollection.FindMemberByContactNumber(contactNumber) != null)
            {
                Console.WriteLine("Invalid contact number. It must be at least 10 digits long, contain only numbers, and must be unique.");
                return;
            }
    ...
    static void RemoveMember(MemberCollection memberCollection)
    {
        try
        {
            ...
            if (memberCollection.FindMemberByName(firstName, lastName)?.GetBorrowedMovies().Length > 0)
            {
                Console.WriteLine("Member cannot be removed as they have borrowed movies.");
                return;
            }
...
class MovieCollection
{
    ...
    public void AddMovie(Movie movie)
    {
        ...
        {
            throw new InvalidOperationException("Movie collection has reached its maximum capacity of 1000 movies.");
        }
...
class Member
{
    ...
    public bool BorrowMovie(string title, MovieCollection movieCollection)
    {
        ...
        {
            Console.WriteLine("You have already borrowed the maximum number of movies.");
            return false;
        }

        if (Array.IndexOf(borrowedMovies, title) >= 0)
        {
            Console.WriteLine("You cannot borrow more than one copy of the same movie.");
            return false;
        }
        ...
        Console.WriteLine("The movie is not available for borrowing.");
        return false;
    }
...
class MemberCollection
{
    ...
    public Member FindMemberByName(string firstName, string lastName)
    {
        ...
        {
            if (members[i] != null && members[i].FirstName.Equals(firstName, StringComparison.OrdinalIgnoreCase) && members[i].LastName.Equals(lastName, StringComparison.OrdinalIgnoreCase))
            {
                return members[i];
            }
        }
    ...
    public Member FindMemberByContactNumber(string contactNumber)
    {
        for (int i = 0; i < members.Length; i++)
        {
            if (members[i] != null && members[i].ContactNumber == contactNumber)
            {
                return members[i];
            }
        }
    ...
    public bool IsMovieBorrowed(string title)
    {
        for (int i = 0; i < members.Length; i++)
        {
            if (members[i] != null && Array.IndexOf(members[i].GetBorrowedMovies(), title) >= 0)
    ...
    private int GetMemberIndex(string firstName, string lastName)
    {
        ...
        while (members[index] != null && !(members[index].FirstName.Equals(firstName, StringComparison.OrdinalIgnoreCase) && members[index].LastName.Equals(lastName, StringComparison.OrdinalIgnoreCase)))
        {
            index = (index + 1) % members.Length;
        }

        if (members[index] != null && members[index].FirstName.Equals(firstName, StringComparison.OrdinalIgnoreCase) && members[index].LastName.Equals(lastName, StringComparison.OrdinalIgnoreCase))
        {
            return index;
        }
